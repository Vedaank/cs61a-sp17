...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        ind = 0
        while ((ind < self.min_range or ind > self.max_range) or (not bees_place.bees)) and bees_place.entrance != hive:
            bees_place = bees_place.entrance
            ind += 1
        return random_or_none(bees_place.bees)
        # END Problem 3Bok -q 05
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            # Attacks all the bees in the same place
            bee_dist = list(self.place.bees)
            for b in bee_dist:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6A
        bee_dist = len(self.place.bees) - 1
        while bee_dist >= 0:
            self.place.bees[bee_dist].reduce_armor(self.damage)
            bee_dist -= 1
        # END Problem 6A
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 9
        def buff(ant):
            if isinstance(ant, BodyguardAnt) and ant.ant != None and ant.ant not in self.already_buffed:
                ant.ant.damage = ant.ant.damage * 2
                self.already_buffed.append(ant.ant)
                if isinstance(ant, TankAnt) and ant not in self.already_buffed:
                    ant.damage = ant.damage * 2
            else:
                if ant not in self.already_buffed:
                    ant.damage = ant.damage*2
        ...
        if self.true_queen == False:
            ...
            while current_place != None:
                if current_place.ant == None:
                    ...
                    buff(current_place.ant)
                    self.already_buffed.append(current_place.ant)
                    current_place = current_place.exit
        # END Problem 9
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 9
        "*** REPLACE THIS LINE ***"
        if amount >= self.armor and self.true_queen == True:
            bees_win()
        Insect.reduce_armor(self, amount)
        # END Problem 9
...
def make_stun(action):
    ...
    """
    # BEGIN Problem EC
    def stunner_action(colony):
        pass
    return stunner_action
    # END Problem EC
