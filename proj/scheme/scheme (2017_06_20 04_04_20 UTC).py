...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        s_eval = scheme_eval(first, env)
        check_procedure(s_eval)
        operands = rest.map(lambda oper: scheme_eval(oper, env))
        return scheme_apply(s_eval, operands, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals or vals:
            raise SchemeError('The length of formals and vals are not equal')
        # END PROBLEM 11
        return child
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        oper = operands.map(lambda operator: scheme_eval(operator, env))
        return scheme_apply(scheme_eval(self.name, env), oper, env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        if type(operand) is int:
            ...
            env.define(target, scheme_eval(operand, env))
        return target 
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        lambda_proc = LambdaProcedure(target.second, expressions.second, env)
...
def do_and_form(expressions, env):
    ...
    expr_first = scheme_eval(expressions.first, env)
    if expr_first is False: 
        return expr_first
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    if expressions is nil:
        return scheme_falsep(expressions)
    elif expressions.second is nil:
        return scheme_eval(expressions.first, env, True)

    expr_first = scheme_eval(expressions.first, env)
    if expr_first is not False:
        return expr_first
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            oper = rest.map(lambda expr_rest: scheme_optimized_eval(expr_rest, env))
            result = scheme_apply(proc, oper, env)
            # END Extra Credit
    return result
