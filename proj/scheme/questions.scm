...
(define (enumerate s)
  ; BEGIN PROBLEM 17
  (define (indexer curr len) 
    (if (< curr len)
      (cons curr (indexer (+ curr 1) len))
      nil
    )
  )
  (zip (list (indexer 0 (length s)) s))
)
  ; END PROBLEM 17
...
(define (list-change total denoms)
  ...
  (if (null? denoms)
    nil
    (cond ((> (car denoms) total) 
            (list-change total (cdr denoms)))
          ((equal? total (car denoms))
         ...
         (let ((values (cadr expr))
           ...
           (begin
            ...
            (define values (cadr (zip values)))
              (cons (append (list 'lambda variables) (let-to-lambda body)) (let-to-lambda values))
           )
           ; END PROBLEM 19
           ))
        (else
