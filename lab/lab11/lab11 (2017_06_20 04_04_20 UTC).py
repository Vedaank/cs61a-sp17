...
def interleave(stream1, stream2):
    ...
    1
    """
    def interleave_helper(one_stream, two_stream, flag):
        if one_stream is None:
            return Stream(two_stream)
        elif two_stream is None:
            return Stream(one_stream)
        elif flag == 0:
            return Stream(one_stream.first, lambda: interleave_helper(one_stream.rest, two_stream, 1))
        elif flag == 1:
            return Stream(two_stream.first, lambda: interleave_helper(one_stream, two_stream.rest, 0))
    return interleave_helper(stream1, stream2, 0)
...
def add_streams(s1, s2):
    ...
    14
    """
    def add_streams_helper(one_stream, two_stream):
        if one_stream is None:
            return Stream(two_stream)
        elif two_stream is None:
            return Stream(one_stream)
        else:
            return Stream(one_stream.first + two_stream.first, lambda: add_streams_helper(one_stream.rest, two_stream.rest))
    return add_streams_helper(s1, s2)
