...
def find_duplicates_k(k, lst):
    ...
    if lst == [] or len(lst) < k:
        	return False
    elif len(set(lst[0:(k+1)])) != len(lst[0:(k+1)]):
    	return True
    else:
    	return find_duplicates_k(k, lst[1:])
...
def scale_stream(s, k):
    ...
    if s is Stream.empty:
        ...
        compute_rest = lambda : scale_stream(s.rest, k)
...
def make_random_stream(seed, a, c, n):
    ...
    ri = mod(add(mul(seed, a), c), n)
    return Stream(seed, lambda: make_random_stream(ri, a, c, n))
