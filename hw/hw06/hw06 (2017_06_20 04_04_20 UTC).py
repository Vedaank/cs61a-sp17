...
def lst_replace_first(L, target, replacement):
    ...
    """
    def replacer(lst):
        if lst is Link.empty:   # Reached end of list without finding TARGET in lst
            raise IndexError    # Raising error to trigger except clause
        elif lst.first == target:
            return Link(replacement, lst.rest)
        else:
            return Link(lst.first, replacer(lst.rest))
    try:
        return replacer(L)
    except IndexError:
        return L
