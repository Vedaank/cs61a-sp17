...
def simplify(expr):
    ...
    if expr.is_leaf():
        return expr
    left = simplify(expr.branches[0])
    right = simplify(expr.branches[1])
    if expr.oper == '+':
        ...
        if left == right:
            ...
            return left
    return Expr(expr.oper, left, right)
...
class VendingMachine:
    ...
    'Here is your soda.'
    """
    def __init__(self, vendable, price):
    ...
    def vend(self):
        if self.stock > 0 and self.balance == self.price:
            self.balance = 0
            self.stock -= 1
            return 'Here is your ' + str(self.vendable) + '.'
        elif self.stock > 0 and self.balance > self.price:
            change = self.balance - self.price
            self.balance = 0
            self.stock -= 1
            return 'Here is your ' + str(self.vendable) + ' and $' + str(change) + ' change.'
        elif self.stock == 0:
    ...
    def restock(self, quantity):
        self.stock += quantity
        return 'Current ' + str(self.vendable) + ' stock: ' + str(self.stock)
